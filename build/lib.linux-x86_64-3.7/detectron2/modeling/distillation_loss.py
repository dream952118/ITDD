...
def backbone_loss(features, prev_features):
    p2 = feature_distillation(features['p2'], prev_features['p2'])
    p3 = feature_distillation(features['p3'], prev_features['p3'])
    p4 = feature_distillation(features['p4'], prev_features['p4'])
    p5 = feature_distillation(features['p5'], prev_features['p5'])
    p6 = feature_distillation(features['p6'], prev_features['p6'])
    loss = p2 + p3 + p4 + p5 + p6
    # loss = feature_distillation(features, prev_features)
    # print(loss)
